...
module.exports = {
    ...
    getAllTimetables : function(type, route, compactify) {
        return this._loadAllTimetables(type, route)
            .then(function(res) {
                console.log('---- loaded all timetables', JSON.stringify(res).substr(0, 50));
                var q = utils.map(res, this._processTimetable, this);
                console.log('---> ', q);
                return q;
            }.bind(this))
            .then(this._mergeDays.bind(this), function(e) { console.log('Error:', e); return 'error!'; })
            .then(compactify? this.compactifyAllTimetables.bind(this) : function(res) { return res; });
    },
    ...
    _processTimetable : function(rawTimetableForDayByDirection) {
        console.log('---- going to process timetable', JSON.stringify(rawTimetableForDayByDirection).substr(0, 50));
        var first = rawTimetableForDayByDirection['A'] || rawTimetableForDayByDirection['B'],
            directions = {},
            res = {
                data : extend(this._getDataFromRawTimetable(first), { stops : {} })
            };
            
        utils.each(rawTimetableForDayByDirection, function(rawTimetable, direction) {
            console.log('- processing', JSON.stringify(rawTimetable).substr(0, 50));
            this._getIteratableStops(rawTimetable).forEach(function(stop, stopIndex) {
                console.log('-- processing stop ', stopIndex, ': ', JSON.stringify(stop).substr(0, 50));
                this._splitStopIntoDirections(this._getStopData(stop), direction, direction + stopIndex, directions);
            }, this);
        }, this);
        
        console.log('---- had split stops', JSON.stringify(directions).substr(0, 50));

        utils.each(directions, function(timesForDirection, direction) {
            console.log(' - sorting stops with', this._sortStops && this._sortStops.toString());
            this._sortStops(timesForDirection);
            console.log(' - times for direction ', direction, ' are ', JSON.stringify(timesForDirection).substr(0, 50));
        ...
        console.log('---- processed timetable', JSON.stringify(res).substr(0, 50));
        return res;
    },
