...
module.exports = {
    ...
    getAllTimetables : function(routeId) {
        ...
        return utils.repeatUntilTrue(function() {
            ...
            return that.getTimetableForDay(routeId, firstEmpty).then(function(tts) {
                if(!tts) return true;
                ...
                res.data[dow] = tts.data;
                delete res.data[dow].dow;
                delete tts.data;
                res[dow] = tts;
            });
        }).then(function() {
    ...
    return extend(
        {
            data : {
                dow : first.dow,
                stops : timesForDayByDirection && utils.map(timesForDayByDirection, function(timesForDirection) {
                    return timesForDirection && timesForDirection.map(function(stop) {
                        return { id : stop.id, title : stop.title };
                    });
                })
            }
        },
        Object.keys(timesForDayByDirection).reduce(function(readyTimesForDayByDirection, direction) {
            timesForDayByDirection[direction] && timesForDayByDirection[direction].schedule && (readyTimesForDayByDirection[direction] = utils.map(timesForDayByDirection[direction].schedule, function(stop) {
                return utils.map(stop.planning, function(timesForHour, hour) {
