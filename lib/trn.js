...
function compactifyStops(stopsByDirection) {
    console.log('compactifying stops', JSON.stringify(stopsByDirection));
    var titles = [],
...
function compactifyAllTimetables(timetablesByDay) {
    return utils.map(timetablesByDay, function(data, key) {
        console.log('compactifying day', key, JSON.stringify(data));
        return key == 'data'?
            extend(data, compactifyStops(data.stops)) :
            utils.map(timetableForDay, function(direction, key) {
                return key == 'data'? 
                    direction :
                    compactifyTimetable(timetableForDirection);
            });
    })
}
